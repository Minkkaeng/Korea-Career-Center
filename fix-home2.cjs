...
const startStr = '<section className="sticky top-0 w-full h-screen bg-[#050B14] flex items-center justify-center overflow-hidden z-0">';
const endStr = '가장 혁신적인 성장의 길을 안내합니다.\n            </motion.p>';
// Try with \r\n as well
const endStr2 = '가장 혁신적인 성장의 길을 안내합니다.\r\n            </motion.p>';

const startIdx = f.indexOf(startStr);
let endIdx = f.indexOf(endStr);
let len = endStr.length;

if (endIdx === -1) {
  endIdx = f.indexOf(endStr2);
  len = endStr2.length;
}

if (startIdx !== -1 && endIdx !== -1) {
  const newHero = `<section className="sticky top-0 w-full h-screen bg-slate-900 flex items-center justify-center overflow-hidden z-0">
  ...
  f = f.substring(0, startIdx) + newHero + f.substring(endIdx + len);
